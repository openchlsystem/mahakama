...
te["tag_calv"] = { div:["c b05 r05","va"], c:[ { div:["gb cw",""], c:
[
	{ s:["c x07 y",null] },
...
function cal_val (p)
{
	...
	{
		window[p.parentNode.nextSibling.firstChild.id] (p.parentNode.nextSibling.firstChild, [v_]);
	}
}
...
function cali (el, k, v, tmf, i, lbl) 
{
	...
	var f = 0;
	var dt = new Date();
	if (v.length>0 && v>0) { dt = new Date ((1*(v+"000"))); f=1; }
