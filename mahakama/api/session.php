...
session_name ("MAHAKAMA_SESSION_ID");
session_start ();
...
function changeAuth (&$o, &$p)
{
	...
	$rt = rest_uri_get ($u, "", $p["profile_id"], $fo_, $p, $aa);
	if ($rt==200) 
	{
		return rest_uri_response ($u, "", $p["profile_id"], $o_, $p, $aa, 202);
	}
...
function changeAuthAdmin ($user_id)
{
	...
	$newpass = _rands (14,"ascii"); // generate random pass
